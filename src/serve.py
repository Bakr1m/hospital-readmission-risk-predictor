...
RISK_THRESHOLDS = {"low": 0.1, "medium": 0.3, "high": 0.5}
...
def get_top_shap_features(shap_values, feature_names, top_k=5):
    ...
    return [
        {"feature": feature_names[i], "shap_value": float(mean_abs_shap[i])} for i in top_indices
    ]
...
def predict(patient: PatientInput):
    try:
        ...
        return PredictionResponse(
            readmission_risk=float(proba), risk_category=risk_cat, top_shap_features=top_features
        )
